...
public class ProcesaMantencion extends Thread {
	...
	private void send2Peya(ProductosFasa pf) {
		try {
			...
			case 0:
				// modificar
				modificaProducto(productsClient, pf.factoryProducto(), pf.getVendorId());
				break;
			case 1:
				// nuevo
			...
			String funcion[] = {"modificar","crear","eliminar"};
			logger.error(String.format("send2Peya: al %s el producto peya: %s", funcion[pf.getTipo()], pf.factoryProducto()), e);
		}
	}
	...
	private Product modificaProducto(ProductsClient productsClient, Product producto, String vendorId) throws ApiException {
		...
		if (producto1 != null) {
			...
			logger.info(String.format("modificaProducto: producto modificado: %s", JsonUtilities.getInstance().toJsonString(Product.class, producto1)));
		}
		return producto1;
	}

	private Product creaProducto(ProductsClient productsClient, Product producto, String vendorId) throws ApiException {
		// crea
		Product producto1 = (Product) productsClient.create(producto, producto.getIntegrationCode());
		if (producto1 != null) {
			logger.info(String.format("creaProducto: producto creado: %s", JsonUtilities.getInstance().toJsonString(Product.class, producto1)));
		}
		return producto1;
	}

	private Boolean eliminaProducto(ProductsClient productsClient, Product producto, String vendorId) throws ApiException {
		// elimina
		boolean resultado = productsClient.delete(producto);
		logger.info(String.format("eliminaProducto: producto %s eliminado %s", resultado?"NO":"", producto));
		return resultado;
	}
